...
class NewsRecUtil:
    ...
    def _optimized_batch_generator(self, train_set, npratio: int, batch_size: int) -> Generator[Dict[str, np.ndarray], None, None]:
        ...
        for user_idx in train_set_user_indices:
            try:
                ...
                # Check if user has at least positive ratings
                if user_idx in self.impressionRating["positive_rating"]:

                    train_pos_items = self.impressionRating["positive_rating"][user_idx]
                    train_neg_items = self.impressionRating["negative_rating"].get(user_idx, [])

                    # Fall back to global negative sampling when no explicit negatives exist
                    if len(train_neg_items) == 0:
                        pos_set = set(train_pos_items)
                        all_item_indices = list(self.item_idx2id.keys())
                        global_negs = [i for i in all_item_indices if i not in pos_set]
                        train_neg_items = global_negs if global_negs else all_item_indices
    ...
    def _get_cached_user_history(self, user_idx: int) -> np.ndarray:
        ...
        if user_idx not in self.user_history_cache:
            ...
            raw_IID = self.user_history.get(raw_UID, [])
    ...
    def map_news_titles_to_Cornac_internal_ids(self, train_set, news_original_id_to_news_title: Dict[Any, str]) -> Dict[int, str]:
        ...
        if not missing_keys:
            print("[OK] All keys in item_id2idx are present in feature_map.")
        else:
            print(f"[WARN] Missing keys in feature_map: {len(missing_keys)} items")
    ...
    def init_news(self, news_title_json: Dict[Any, str]) -> None:
        ...
        print(f"[OK] Initialized {len(news_title_tokens)} news titles")
    ...
    def optimize_memory_usage(self) -> None:
        ...
        print(f"[OK] Optimized memory usage - cache limit: {self.max_cache_size}, batch limit: {self.batch_memory_limit}")
