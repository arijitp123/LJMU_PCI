...
class Sample_And_Rank(object):
    ...
    def filterHeuristics(self, user_idx, itemPool, filteringCriteria, given_item_pool=None):
       ...
       # 1)Based on attributes. E.g., getting rid of articles older than X hours.
        ...
        row = self.train_set_rating.getrow(user_idx) if hasattr(self.train_set_rating, 'getrow') else self.train_set_rating[user_idx]
        if hasattr(row, 'toarray'):
            historyArticles = np.where(row.toarray().flatten() >= 1)[0]
        else:
            historyArticles = np.where(row == 1)[0]
    ...
    def sampleArticles(self, targetDimensions, targetDistributions, targetSize, linear_program_coefficient):
        ...
        # Temporary debug — inspect LP cost vector scale before PCI penalty
        print(f"[DEBUG] C scale: min={C.min():.4f} max={C.max():.4f} mean={C.mean():.4f}")

        # PCI adaptive: penalize items from overrepresented categories
        if hasattr(self, '_pci_active') and self._pci_active and self._category_penalties:
            cat_col = self.pci_config.get("category_column", "category")
            penalty_weight = self.pci_config.get("penalty_weight", 2.0)
            for idx_pos, item_idx in enumerate(self.itemPool):
                if item_idx in self.articlesDataframe.index:
                    item_cat = self.articlesDataframe.loc[item_idx, cat_col]
                    if pd.notna(item_cat) and item_cat in self._category_penalties:
                        proportion = self._category_penalties[item_cat]
                        # Add penalty proportional to category's share in user history.
                        # Since linprog minimizes C, adding positive values makes
                        # overrepresented-category items less preferred.
                        C[idx_pos] += penalty_weight * proportion
    ...
    def newHop(self, user_id, targetDimensions, targetDistributions, targetSize, sampleObjective, currentHop, filteringCriteria, given_item_pool=None):
        ...
        row = self.train_set_rating.getrow(user_id) if hasattr(self.train_set_rating, 'getrow') else self.train_set_rating[user_id, :]
        if hasattr(row, 'nnz'):
            isEmptyHistory = (row.nnz == 0)
        else:
            isEmptyHistory = np.all(row == 0)
        if isEmptyHistory:
    ...
    def performSampling(self, user_id, listSize, targetDimensions, targetDistribution,   maxHops, filteringCriteria,  sampleObjective,  rankingType, rankingObjectives, mappingList, ascending, given_item_pool=None):
        ...
        # PCI adaptive mechanism: check if this user's PCI exceeds threshold
        self._pci_active = False
        self._category_penalties = None
        if hasattr(self, 'pci_config') and self.pci_config and self.pci_config.get("enabled", False):
            if hasattr(self, 'user_pci_values') and user_id in self.user_pci_values:
                user_pci = self.user_pci_values[user_id]
                threshold = self.pci_config.get("threshold", 0.5)
                if user_pci is not None and user_pci > threshold:
                    self._pci_active = True
                    # Compute category distribution from user's history
                    cat_col = self.pci_config.get("category_column", "category")
                    row = self.train_set_rating.getrow(user_id) if hasattr(
                        self.train_set_rating, 'getrow') else self.train_set_rating[user_id]
                    if hasattr(row, 'toarray'):
                        history_items = np.where(row.toarray().flatten() >= 1)[0]
                    else:
                        history_items = np.where(row >= 1)[0]
                    from collections import Counter
                    hist_cats = [
                        self.articlesDataframe.loc[i, cat_col]
                        for i in history_items
                        if i in self.articlesDataframe.index
                        and pd.notna(self.articlesDataframe.loc[i, cat_col])
                    ]
                    if len(hist_cats) > 0:
                        cat_counts = Counter(hist_cats)
                        total = len(hist_cats)
                        self._category_penalties = {
