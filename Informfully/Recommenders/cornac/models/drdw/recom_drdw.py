...
class D_RDW(Recommender):
    ...
    def fit(self, train_set, val_set=None):
       ...
       """
        ...
        self.train_set_rating = gt_mat

        self.sampleRank = Sample_And_Rank(
            self.train_set_rating, self.item_dataframe)

        # PCI adaptive mechanism: pre-compute per-user PCI from training history
        self.sampleRank.pci_config = self.pci_config
        self.sampleRank.user_pci_values = {}
        if self.pci_config and self.pci_config.get("enabled", False):
            self._recompute_all_pci()

        return self

    def _recompute_all_pci(self):
        """Recompute PCI values for all users from current train_set_dict."""
        from collections import Counter
        import pandas as pd
        cat_col = self.pci_config.get("category_column", "category")
        min_history = self.pci_config.get("min_history", 5)
        self.sampleRank.user_pci_values = {}
        for user_idx, items in self.train_set_dict.items():
            if len(items) < min_history:
                continue
            categories = [
                self.item_dataframe.loc[i, cat_col]
                for i in items
                if i in self.item_dataframe.index
                and pd.notna(self.item_dataframe.loc[i, cat_col])
            ]
            if len(categories) > 0:
                counts = Counter(categories)
                total = len(categories)
                self.sampleRank.user_pci_values[user_idx] = sum(
                    (c / total) ** 2 for c in counts.values()
                )

    def update_history(self, user_idx, new_item_indices):
        """
        Update a user's history with newly consumed items and rebuild internal structures.
        Used for multi-session simulation where user history grows over time.

        Parameters
        ----------
        user_idx : int
            The Cornac internal user index.
        new_item_indices : list of int
            Cornac internal item indices the user consumed.
        """
        from scipy.sparse import lil_matrix

        # 1. Update train_set_dict
        if user_idx not in self.train_set_dict:
            self.train_set_dict[user_idx] = []
        self.train_set_dict[user_idx].extend(new_item_indices)

        # 2. Update CSR matrix (CSR -> LIL -> modify -> CSR)
        lil = self.train_set_rating.tolil()
        for item_idx in new_item_indices:
            if item_idx < lil.shape[1]:
                lil[user_idx, item_idx] = 1.0
        self.train_set_rating = lil.tocsr()

        # 3. Rebuild Sample_And_Rank (new bipartite graph from updated matrix)
        self.sampleRank = Sample_And_Rank(
            self.train_set_rating, self.item_dataframe)
        self.sampleRank.pci_config = self.pci_config

        # 4. Recompute all PCI values from updated history
        if self.pci_config and self.pci_config.get("enabled", False):
            self._recompute_all_pci()
        else:
            self.sampleRank.user_pci_values = {}

    def batch_update_history(self, updates):
        """
        Batch update multiple users' histories, then rebuild graph once.
        Much more efficient than calling update_history() per user.

        Parameters
        ----------
        updates : dict
            {user_idx: [list of new item indices]}
        """
        from scipy.sparse import lil_matrix

        # 1. Update train_set_dict for all users
        for user_idx, new_items in updates.items():
            if user_idx not in self.train_set_dict:
                self.train_set_dict[user_idx] = []
            self.train_set_dict[user_idx].extend(new_items)

        # 2. Batch update CSR matrix
        lil = self.train_set_rating.tolil()
        for user_idx, new_items in updates.items():
        ...
        # 3. Rebuild Sample_And_Rank once
        self.sampleRank = Sample_And_Rank(
            self.train_set_rating, self.item_dataframe)
        self.sampleRank.pci_config = self.pci_config

        # 4. Recompute all PCI values
        if self.pci_config and self.pci_config.get("enabled", False):
            self._recompute_all_pci()
        else:
            self.sampleRank.user_pci_values = {}
    ...
    def rank(self, user_idx, item_indices=None, k=-1, **kwargs):
        ...
        num_train_items = self.train_set_rating.shape[1]
        item_indices = (
        list(np.arange(self.num_items))
        if item_indices is None
        else list(item_indices)
        )
        # Filter out items beyond the training set (unknown items)
        known_item_indices = [i for i in item_indices if i < num_train_items]
        ...
        ranked_items, ranked_item_scores = self.sampleRank.performSampling(
            user_idx, self.targetSize,  self.diversity_dimension, selectedTarget, self.maxHops,  self.filteringCriteria, self.sampleObjective, self.rankingType,  self.rankingObjectives, self.mappingList, self.ascending, given_item_pool=known_item_indices)

        # Build scores for all requested item_indices (0 for unknown items)
        rdw_scores = self.sampleRank.articleRdwScore
        random_walk_prob = np.array([
            rdw_scores[i] if i < len(rdw_scores) else 0.0
