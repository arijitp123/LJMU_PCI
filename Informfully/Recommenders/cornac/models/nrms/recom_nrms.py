...
class NRMS(Recommender):
    ...
    def fit(self, train_set, val_set=None):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if step == 0:
                print(f"Warning: Epoch {epoch} produced no training batches. Check training data has valid user interactions.")
                break
    ...
    def score(self, user_idx, item_idx=None, **kwargs):
        ...
        # Cache id<->idx mappings (rebuilt once, not on every call)
        if not hasattr(self, '_item_idx2id') or self._item_idx2id is None:
            self._item_idx2id = {v: k for k, v in self.iid_map.items()}
            self._user_idx2id = {v: k for k, v in self.uid_map.items()}
        self.item_idx2id = self._item_idx2id
        self.user_idx2id = self._user_idx2id
        ...
        if isinstance(item_idx, (int, np.integer)):
            ...
            raise Exception("item_idx should be an int, list, or numpy array")

        # --- Pre-compute and cache all news vectors (runs once per evaluation) ---
        if not hasattr(self, '_cached_news_vecs') or self._cached_news_vecs is None:
            all_titles = np.array(
                self.news_organizer.news_title_index, dtype=np.int64
            )  # shape: (total_news, title_size)
            self._cached_news_vecs = self.newsencoder.predict(
                all_titles, batch_size=512, verbose=0
            )  # shape: (total_news, embed_dim)

        # Map cornac item indices to news_title_index positions
        item_news_indices = [
            self.news_organizer.news_index_map[self.item_idx2id[idx]]
            for idx in item_idx
        ]
        candidate_vecs = self._cached_news_vecs[item_news_indices]  # (N, embed_dim)
        ...
        if user_idx in self.news_organizer.click_title_all_users:
            ...
            # Handle unknown/cold-start user
            raw_UID = self.user_idx2id[user_idx]
            raw_IID = self.userHistory.get(raw_UID, [])
            click_title_index = self.news_organizer.process_history_news_title(raw_IID, self.history_size)

        # Encode user once (shape: (1, history_size, title_size) -> (1, embed_dim))
        user_input = np.expand_dims(np.asarray(click_title_index, dtype=np.int64), axis=0)
        user_vec = self.userencoder.predict(user_input, batch_size=1, verbose=0)[0]  # (embed_dim,)

        # Score = dot(candidate_vecs, user_vec) passed through sigmoid
        raw_scores = candidate_vecs @ user_vec  # (N,)
        final_predictions = 1.0 / (1.0 + np.exp(-raw_scores))
