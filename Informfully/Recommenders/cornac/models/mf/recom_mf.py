...
class MF(Recommender, ANNMixin):
    ...
    * Koren, Y., Bell, R., & Volinsky, C. Matrix factorization techniques for recommender systems. \
    ...
    def _fit_cpu_python(self, train_set, val_set, rid, cid, val):
        """Pure Python fallback for CPU fitting when Cython is unavailable"""
        ...
        num_ratings = len(rid)
        num_factors = self.u_factors.shape[1]
        
        for iteration in trange(self.max_iter, desc="Fitting MF", disable=not self.verbose):
            for idx in range(num_ratings):
                u = int(rid[idx])
                i = int(cid[idx])
                rating = float(val[idx])
                
                # Compute prediction
                pred = self.global_mean + self.u_biases[u] + self.i_biases[i]
                pred += np.dot(self.u_factors[u], self.i_factors[i])
                ...
                # Update biases
                if self.use_bias:
                    self.u_biases[u] += self.learning_rate * (error - self.lambda_reg * self.u_biases[u])
                    self.i_biases[i] += self.learning_rate * (error - self.lambda_reg * self.i_biases[i])
                
                # Update factors
                u_factor_old = self.u_factors[u].copy()
                self.u_factors[u] += self.learning_rate * (error * self.i_factors[i] - self.lambda_reg * self.u_factors[u])
                self.i_factors[i] += self.learning_rate * (error * u_factor_old - self.lambda_reg * self.i_factors[i])
    ...
    def _fit_cpu(self, train_set, val_set):
        ...
        if use_cython:
            try:
                ...
                # Fall back to pure Python if Cython has type issues
                use_cython = False
        
        if not use_cython:
            self._fit_cpu_python(train_set, val_set, rid, cid, val)
