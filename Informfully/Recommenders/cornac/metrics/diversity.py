...
class PCI(DiversityMetric):
    """Perspective Concentration Index.

    Measures how concentrated a user's exposure to different perspectives is,
    based on the Herfindahl-Hirschman Index (HHI) applied to category
    proportions across the user's recommended items and reading history.

    PCI(u) = sum(s_k^2) for k=1..K

    Where s_k is the proportion of items from perspective k in the user's
    combined recommendation list and history. The score ranges from 1/K
    (perfectly diverse across K categories) to 1.0 (all items from a single
    category, i.e., an echo chamber).

    Parameters
    ----------
    item_categories : dict
        A dictionary mapping item indices (int) to category labels (str).
        Example: {0: "sports", 1: "politics", 2: "sports", ...}
    k : int, optional, default: -1
        The number of items in the top@k recommendation list to consider.
        If -1, all recommended items are used.
    ...
    def __init__(self, item_categories, k=-1):
        DiversityMetric.__init__(self, name="PCI@{}".format(k), k=k)
        if not isinstance(item_categories, dict):
            raise ValueError(
                "PCI received an invalid item_categories. "
                "item_categories must be a dictionary mapping item indices "
                "to category strings."
            )
        self.item_categories = item_categories

    def compute(self, pd_rank, user_history=None, **kwargs):
        """Compute PCI score for a single user.

        Parameters
        ----------
        pd_rank : numpy array
            Ranked array of recommended item indices.
        user_history : numpy array or list, optional
            Item indices the user interacted with in training data.
        **kwargs : for compatibility

        Returns
        -------
        float or None
            PCI score. Higher values indicate more concentration.
            Returns None if no category data is available.
        """
        if self.k > 0:
            truncated_pd_rank = pd_rank[:self.k]
        else:
        ...
        # Combine recommendations and user history
        all_items = list(truncated_pd_rank)
        if user_history is not None:
            all_items = all_items + list(user_history)
        ...
        # Count proportions per category
        cat_counts = Counter(categories)
        total = len(categories)
        proportions = [count / total for count in cat_counts.values()]

        # PCI = sum of squared proportions (Herfindahl-Hirschman Index)
        pci = sum(p ** 2 for p in proportions)
        return pci
